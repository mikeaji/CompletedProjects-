...
public class CPC_CameraPath : MonoBehaviour
{
    ...
    void Start ()
    {
        ...
        if (playOnAwake)
        {
            PlayPath(playOnAwakeTime);//start camera rotation
            PausePath();//pause camera rotation
        }
    }

     void Update()
    {
        if (GameController.instance.IsRotationallowed && !GameController.instance.SkipBtnWasPressed )//if nobody's turn (roattion allowed) and camera btn is not pressed 
        {
            GameController.instance.light.SetActive(true);//turn roating camera's light on
            ResumePath();//and play the rotation
        }
        else
        {
            GameController.instance.light.SetActive(false);// turn roating camera's light off
            PausePath();//and pause the rotation
        }

        if (GameController.instance.SkipBtnWasPressed)//if camera btn was pressed
        {
            //current change waypoints
        ...
        if (GameController.instance.resumedAtTur2)//game is resumned at p2's turn
        {
